# Healthy State Testing

## Objective
Verify the system correctly classifies and reports a **Healthy** machine condition end-to-end (sensors → signal processing → TinyML → state machine → OLED/LED/buzzer → MQTT).

## Test Procedure
1. Place the sensor rig on a machine (or simulated source) exhibiting normal/healthy operating characteristics.
